    ...
    <article class="dest-card" data-region="${d.region}" data-country="${d.country.toLowerCase()}">
      ...
      <div class="body">
        ...
        <div class="foot">
          ...
          <a class="arrow" href="index.html?dest=${d.id}#mapa" aria-label="Ver detalles de ${d.uni}">
...
function bootMap() {
  ...
  // Only run map-panel logic on pages that actually have the map
  if (document.getElementById("map-panel")) {
    // If we arrived from another page with ?dest=ID, select & scroll to that destination
    ...
    if (wanted && DESTINATIONS.some(d => d.id === wanted)) {
      ...
      // wait for the async world map to finish drawing markers
...
// Bundled/standalone builds inject this script after DOMContentLoaded has
// already fired, so that event alone can't be relied on — run immediately
// if the DOM is already parsed, otherwise wait for it as usual.
